$(document).ready(function () {
  ...
  function showMessage(message, type = "error") {
    ...
    $("#error-modal-overlay")
      .removeClass("error-modal-hidden")
      .addClass("open");
  }
  ...
  function renderTeamId(teamId) {
    if (teamId) {
      $("#team-id-display")
        .html(
          `<div class="flex items-center justify-between gap-2">` +
            `<div><strong>Team ID:</strong><br>${teamId}</div>` +
            `<button type="button" id="btn-clear-team" class="text-xs text-muted-foreground hover:text-destructive underline flex-shrink-0">Clear</button>` +
            `</div>`,
        )
        .removeClass("hidden");
      ...
      $("#btn-clear-team")
        ...
        .on("click", function () {
          currentTeamId = null;
          teamStorage.clearStoredTeamId();
          renderTeamId(null);
          checkAIAvailability();
        });
    } else {
      $("#team-id-display").addClass("hidden").empty();
    }
  }

  const storedTeamId = teamStorage.getStoredTeamId();
  if (storedTeamId) {
    currentTeamId = storedTeamId;
    renderTeamId(currentTeamId);
    checkAIAvailability();
  }
  ...
  $("#register-form").on("submit", function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        teamStorage.setStoredTeamId(currentTeamId);
        renderTeamId(currentTeamId);
        checkAIAvailability();
      },
      error: function (xhr) {
  ...
  function loadRounds(sessionId) {
    $.get(`/api/contender/rounds?sessionId=${sessionId}`, function (rounds) {
      ...
      rounds.forEach((r) => {
        select.append(
          `<option value="${r.roundNo}">Round ${r.roundNo} (${r.status})</option>`,
        );
      });
        ...
        .on("change", function () {
          ...
          if (roundNo) {
            ...
            $("#round-constraints-container").html(
              '<p class="text-muted-foreground">Select a round to view constraints.</p>',
            );
            activeRoundNo = null;
  ...
  function loadRoundConstraints(roundNo) {
    $.get(
      `/api/contender/round/${roundNo}?sessionId=${currentSessionId}`,
      function (spec) {
        ...
        if (spec.requiredRoles)
          Object.entries(spec.requiredRoles).forEach(
            ([k, v]) =>
              (rolesHtml += `<span class="bg-secondary px-2 py-0.5 rounded mr-1">${v}x ${k}</span>`),
          );

        let bansHtml = "";
        if (spec.bannedTags)
          spec.bannedTags.forEach(
            (t) =>
              (bansHtml += `<span class="text-destructive bg-destructive/10 px-2 py-0.5 rounded mr-1">${t}</span>`),
          );
  ...
  function loadHeroes() {
    ...
    $("#hero-list").html(
      '<div class="flex justify-center py-8"><div class="loader w-8 h-8"></div></div>',
    );
    ...
    $.get(`/api/contender/heroes`, params, function (heroes) {
      ...
      if (!heroes || heroes.length === 0) {
        list.html(
          '<div class="text-center py-12 text-muted-foreground">No heroes found matching your criteria.</div>',
        );
  ...
  function updateSquadUI() {
    ...
    if (selectedHeroes.size === 0) {
      container.html(
        '<p class="text-sm text-muted-foreground italic">No heroes selected yet.</p>',
      );
    }
    ...
    if (roundConstraints) {
      if (totalCost > roundConstraints.budgetCap) {
        $("#total-squad-cost")
          .addClass("text-destructive")
          .removeClass("text-primary");
      } else {
        $("#total-squad-cost")
          .removeClass("text-destructive")
      ...
      if (selectedHeroes.size > roundConstraints.teamSize) {
        $("#squad-count")
          ...
          .removeClass("bg-primary/10 text-primary");
      } else {
        $("#squad-count")
          .removeClass("bg-destructive/20 text-destructive")
  ...
  function renderDraftOptions() {
    ...
    currentDraftOptions.forEach((option) => {
      const selected =
        ...
        currentRecommendation.strategyId === option.strategyId;
      const statusClass = option.valid
        ? "border-primary/30"
    ...
    $(".draft-option")
      ...
      .on("click", function () {
        ...
        const option = currentDraftOptions.find(
          (o) => o.strategyId === strategyId,
        );
        if (!option) return;
  ...
  function setSelectedRecommendation(option) {
    ...
    option.recommendation.heroes.forEach((h) => {
      heroesList.append(
        `<li>🦸‍♂️ ${h.name} <span class="text-xs ml-2 opacity-50">(ID: ${h.id})</span></li>`,
      );
    });

    if (!option.valid) {
      heroesList.append(
        `<li class="text-destructive">Violations: ${option.violations.join(" ")}</li>`,
      );
    ...
    $.ajax({
      ...
      success: function (resp) {
        ...
        const recommendedOption =
          currentDraftOptions.find(
            (o) => o.strategyId === resp.recommendedStrategyId,
          ) ||
  ...
  $("#btn-submit-squad").on("click", function () {
    ...
    $("#submit-feedback")
      .removeClass(
        "bg-destructive/20 text-destructive bg-green-500/20 text-green-500",
      )
      .hide();
  ...
  $("#btn-submit-manual-squad").on("click", function () {
    ...
    if (roundConstraints) {
      if (selectedHeroes.size > roundConstraints.teamSize) {
        showMessage(
          `Too many heroes! Max team size is ${roundConstraints.teamSize}.`,
        );
        return;
      }
      let total = 0;
      ...
      if (total > roundConstraints.budgetCap) {
        showMessage(
          `Budget exceeded! Max budget is ${roundConstraints.budgetCap}.`,
        );
        return;
      }
    }
